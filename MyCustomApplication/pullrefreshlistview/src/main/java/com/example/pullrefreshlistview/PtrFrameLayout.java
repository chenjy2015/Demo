...
import com.example.pullrefreshlistview.util.NetWorkUtils;
...
public class PtrFrameLayout extends ViewGroup {
    ...
    //刷新成功的时候header的延时关闭时间
    private long mDelayToCloseHeader;
    ...
    //    准备关闭头部
    private boolean mIsPrepareComplete;
...
//        header刷新成功后延时关闭的时间
        mDelayToCloseHeader = 0;
//        是否刷新成功
        mIsRefreshSuccess = true;
    }
    ...
    protected void onDetachedFromWindow() {
        ...
        PtrCLog.d(TAG, "onDetachedFromWindow");
    ...
    protected void onAttachedToWindow() {
        ...
        PtrCLog.d(TAG, "onAttachedToWindow");
        if (mPtrIndicator.hasLeftStartPosition()) {
            if (mIsPrepareComplete) {
                refreshComplete();
            } else {
                onRelease(false);
            }
    ...
    public boolean dispatchTouchEvent(MotionEvent e) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if ((moveUp && canMoveUp) || (moveDown && mEnableRefresh)) {
                    movePos(offsetY);
                    return false;
                }
        }
    ...
    private void onRelease(boolean stayForLoading) {
        ...
        if (mStatus == PTR_STATUS_LOADING) {
            ...
            if (mStatus == PTR_STATUS_COMPLETE) {
                if (stayForLoading && isAutoRefresh()) {//自动刷新时
                    mDelayToCloseHeader = 350;
                } else {
                    mDelayToCloseHeader = 0;
                }
    ...
    private void tryScrollBackToTopAfterComplete() {
        if (mIsRefreshSuccess || (!mIsRefreshSuccess && (NetWorkUtils.isNetWorkEnable(getContext())
                || NetWorkUtils.isWifiEnable(getContext())))) {//刷新成功 或者刷新失败(有网络)
            if (!mPtrIndicator.isUnderTouch() && !mPtrIndicator.isInStartPosition()) {
                PtrCLog.d(TAG, "延时关闭header");
                if (mDelayToCloseHeader > 0) {
...
//        是否开始结束,Fragment切换造成无法回滚
        mIsPrepareComplete = true;

        PtrCLog.d(TAG, "ptr,请求回来");
        refreshComplete();
    }
    ...
    private void notifyUIRefreshComplete(boolean ignoreHook) {
        ...
        if (mPtrUIHandlerHolder.hasHandler()) {
            ...
            if (mIsRefreshSuccess || (!mIsRefreshSuccess && (NetWorkUtils.isNetWorkEnable(getContext())
                    || NetWorkUtils.isWifiEnable(getContext())))) {
                mPtrUIHandlerHolder.onUIRefreshComplete(this, mIsRefreshSuccess);
            }
        }
    ...
    public void autoRefresh() {
        autoRefresh(true, mDurationToCloseHeader, true);
    }

    public void autoRefresh(boolean atOnce) {
        autoRefresh(atOnce, mDurationToCloseHeader, true);
    }
    ...
    public void autoRefresh(boolean atOnce, int duration, boolean isScroll) {
        ...
        int offsetToRefresh = mPtrIndicator.getOffsetToRefresh();
        if (offsetToRefresh == 0) {
            offsetToRefresh = SizeUtils.dip2px(getContext(), 80);
        }
        if (isScroll) {
            mScrollChecker.tryToScrollTo(offsetToRefresh, duration);
        } else {
            movePos(offsetToRefresh);
        }
        if (atOnce) {
            mStatus = PTR_STATUS_LOADING;
            performRefresh();
...
//            Fragment调用onDestoryView方法的时候,Scroller停止滑动
            if (finish && mPtrIndicator.hasLeftStartPosition() && mPtrIndicator.getCurrentPosY() > mTo) {
