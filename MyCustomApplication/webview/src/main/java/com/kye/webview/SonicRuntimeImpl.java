...
public class SonicRuntimeImpl extends SonicRuntime {
    ...
    public void postTaskToThread(Runnable task, long delayMillis) {
        Thread thread = new Thread(task, "KYEThread");
        thread.start();
    }

    @Override
    public File getSonicCacheDir() {
        String path = Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + mContext.getString(R.string.sdcard_webcache_dir);
